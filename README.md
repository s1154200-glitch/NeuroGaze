...
### ADHD Eye-Tracking Screening Suite
...
- [Calibration Features](#calibration-features)
- [ADHD Tests](#adhd-tests--details)
- [Training Games](#training-games)
- [ADHD Probability Calculation](#adhd-probability-calculation)
...
Serve the files from a local HTTP server to keep the browser origin consistent:
...
Then open **http://localhost:8080/dashboard.html** in Chrome or Edge.
...
4. After all 9 points, a brief accuracy check runs. Calibration is saved if accuracy reaches **≥ 70 %**.
5. Re-calibrate any time accuracy feels off — the button is in the top-right of every test page.

> **Tip:** Good calibration is essential. Poor lighting or a moving head will reduce accuracy.
...
| **CPT (Go/No-Go)** | ~5 min | Commission errors, omission errors, RT variability, gaze wander, attentional decay |
...
| Page | What you see |
|:---|:---|
| **Dashboard** | Latest ADHD probability per test and live biomarker zone indicators |
| **Results** (`results.html`) | Full breakdown with scatter plots, latency histograms, and per-test panels for the most recent session |
| **Analysis** (`analyze.html`) | Trend view across all saved sessions, improvement/regression badges, and AI-generated insights |
...
A face-boundary box and info badge are overlaid on the camera preview. Using 468-point MediaPipe face-mesh landmarks, the system computes your nose-tip position, inter-pupillary distance (IPD), and face bounding box, saving them as a **reference state** at calibration time.
...
| 25 – 50 % | ⚠️ Shifted | Yellow warning banner |
| > 50 % | 🔴 Out of Range | Red banner + auto recheck after 3 s |

The info badge shows real-time **positional offset (px)** and **depth (IPD %)** values alongside an OK / Shifted / Out of Range label.

### Range Check

When enabled, the system monitors face drift from the calibration position. If the **Out of Range** state persists for more than **3 seconds**, an accuracy recheck is triggered automatically — you are asked to look at the centre dot and the gaze model is verified against the known target.

Disable Range Check to suppress warning banners and skip automatic rechecks (useful with a head-mounted display or fixed chin-rest).

### Offset Correction

Raw WebGazer predictions assume your head is in the same position it was during calibration. Offset Correction compensates for head movement in real time using three components:

| Component | What it compensates |
|:---|:---|
| **Position shift** | Horizontal/vertical nose-tip displacement (mirrored) scaled to screen pixels |
| **Depth (IPD ratio)** | Lean closer/farther — gaze prediction contracts/expands from screen centre |
| **Z-depth fine-tune** | Residual nose-landmark z-delta applied at ×0.002 scale |

Disabling Offset Correction passes raw WebGazer coordinates directly to the tests — useful for measuring baseline WebGazer accuracy or matching earlier datasets.

### Tolerance Radius — `tol80`

After calibration, all gaze samples from the accuracy check are sorted by Euclidean distance from the target dot. The **80th-percentile distance** is stored as `tol80` (`localStorage` key: `neurogaze_cal_tol80`).

$$\text{tol80} = \text{sorted\_distances}\!\left[\lfloor n \times 0.8 \rfloor\right]$$

where distances are computed as:

$$d_i = \sqrt{(x_i - c_x)^2 + (y_i - c_y)^2}$$

`tol80` acts as a **personalised gaze tolerance window** that scales to the user's actual webcam accuracy:

| Consumer | How `tol80` is applied | Hard limits |
|:---|:---|:---:|
| Fixation Stability test | Sets `QE_RADIUS_PX` — radius for Quiet Eye scoring | 20 – 300 px |
| Antisaccade test | Sets `lastTol80` for hit detection per trial | — |
...
Users with lower-quality webcams or poor lighting still receive fair hit detection because tolerances expand proportionally to measured gaze error.
...
Look at a stationary dot for 30 seconds. The test measures how stable your gaze is.

| Biomarker | Description |
|:---|:---|
| **BCEA** | Area (degrees²) containing 68 % of gaze samples — higher = less stable |
...
A stimulus flashes on one side of the screen — you must look to the **opposite** side, suppressing a reflexive glance.
...
| **Gaze Wander** | % of time gaze strayed from screen centre |
| **Attentional Decay** | Difference in omission rate between the first and second half |

> **CPT Training Game:** The gamified version (**Goldfish Scooping**) uses a 3-life system — scoop koi fish (Go) and ignore lotus leaves (No-Go). Each mistake costs one life; the game ends when all 3 lives are lost.

### Distractor Recovery Test

Track a smoothly moving dot (figure-8 path). Four times during the task a bright flash appears on the periphery. The test measures how quickly and reliably you return focus after each distractor.
...
| **Off-Task Time** | Total % of task time gaze was outside the target zone |
...
> Games do **not** record data to the test history and do not affect ADHD probability scores.
...
> **Disclaimer:** Scores are indicators derived from webcam-based gaze data and **should not be interpreted as clinical diagnoses**.
...
<details>
<summary><strong>Camera access denied</strong></summary>

Go to your browser's site settings and allow camera access for the current origin (`file://` or `localhost`).
</details>

<details>
<summary><strong>"Calibration not done" shown after calibrating</strong></summary>

Calibration data is stored in `localStorage`. Make sure you open the same file path each session, or use a local server so the origin is consistent.
</details>

<details>
<summary><strong>Gaze dot is far from where I'm looking</strong></summary>
...
<details>
<summary><strong>Tests show NaN or — for all values</strong></summary>
...
Inject realistic fake data for UI testing without running real tests:
...
An internal diagnostic tool for tuning and verifying three low-level eye-tracking systems:

| System | What it checks |
|:---|:---|
| **Tolerance radius** | Visualises the gaze hit radius used by tests to determine whether the user is looking at a target |
| **Calibration accuracy** | Inspects raw gaze predictions against known screen positions to validate calibration quality |
| **Blink detection** | Shows real-time blink events and the artifact-rejection logic that filters eyelid-closing frames |
